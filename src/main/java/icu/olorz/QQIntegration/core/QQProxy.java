...
public class QQProxy implements IQQProxy {
  ...
  public QQProxy(IMinecraftAdapter mcAdapter, IConfig config) {
    // 创建后端服务对象
    if (config.backendType() == Constants.BACKEND_TYPE.COOLQ) {
      this.backendInstance =
          new CoolQWSServer(
              new InetSocketAddress(config.host(), config.port()), this,config);
    } else if (config.backendType() == Constants.BACKEND_TYPE.MIRAI) {
      LOGGER.debug("MiraiBot 初始化");
      this.backendInstance = new MiraiBot(this,config);
    } else {
      this.backendInstance = new MiraiBot(this,config);
    }
